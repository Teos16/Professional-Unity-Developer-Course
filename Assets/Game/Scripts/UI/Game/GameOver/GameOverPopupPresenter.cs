...
namespace Game.UI
{
    public sealed class GameOverPopupPresenter : IGameUIInit, IGameUIDispose
    {
        ...
        {
            _subscription = _gameContext.GetGameFinishedEvent().Subscribe(OnGameFinished);
        ...
        {
            TeamType winner = _gameContext.GetWinner();
        ...
        private void OnRestartClicked() => _appContext?.LoadGame().Forget();
