...
namespace SampleGame.Gameplay
{
    ...
    public sealed class EntitySerializer 
    {
        ...
        public EntitySerializer(EntityWorld world, EntityComponentsSerializer componentsSerializer)
        {
            _world = world;
        ...
        {
            LinkedList<Entity> allEntities = new(_world.GetAll());

            LinkedList<Entity> needlessEntities = CreateEntities(dataSet, allEntities);
            SetEntityProperties(dataSet);
            
            foreach (Entity entity in needlessEntities) 
                _world.Destroy(entity);
        }

        private LinkedList<Entity> CreateEntities(EntityData[] dataSet, LinkedList<Entity> allEntities)
        {
